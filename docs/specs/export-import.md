...
**Status:** **Complete (v1) — implemented; spec tests + GitHub CI 5/5 green on PR #25; not on `main` yet.** All product locks resolved 2026-08-16 (see [§ Product decisions](#product-decisions-locked-2026-08-16)). Mode is **replace**. Code lives in `client/lib/import/` + Settings → **Import data**; the 17 cases in [§ Test expectations](#test-expectations) are in `client/test/import/`. Remaining work is merge. CES-71 stays blocked until this is on `main`.
...
| Import | Settings → **Import data** (`client/lib/import/` + `import_data_section.dart`) replaces local history from a ZIP the app produced. **Not on `main` yet.** Tests live in `client/test/import/` (spec § Test expectations; green on PR #25, GitHub CI 5/5). | Same code on `main` |
