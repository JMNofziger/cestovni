...
- Import is **implemented (CES-70)** — Settings → Backup → **Import data** (`client/lib/import/` + `import_data_section.dart`), replace mode, typed `REPLACE` when local history is non-empty. Spec tests + GitHub CI 5/5 green on PR #25; not on `main` yet. Destructive reset remains Later.
