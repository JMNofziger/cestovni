...
| **3 — Delivery**         | Linear breakdown, estimates, link `docs/specs/...`                                                                                                                                                                                                                                             | **Next**                  |
...
| 2026-04-18 | **Stage 5 Step 10 completed (backlog ready):** Linear `Delivery v1` epic **CES-35** + 12 vertical children **CES-36..CES-47** created with `Spec:` paths and `blockedBy` relations matching the delivery plan. **ADR 003** (mobile stack — Flutter + Drift) and **ADR 004** (telemetry/crash SDK — Sentry self-host-capable) Accepted. **`ci/telemetry-gate.*` live** (`ci/telemetry-schema.json` + `ci/telemetry-gate.py` + `ci/telemetry-gate.yml` + `ci/README.md`): validates the authoritative YAML, bans `pii_class: identifier`/`freetext` at schema level, gracefully skips the client-source + Apple-manifest checks until those land in M1 / pre-submission. Linear project description + `docs/linear/saved-views.md` updated to point at the delivery plan and the Stage 5 view. **Next:** Step 11 milestones (M0 → M5) execute as separate Cursor sessions; Overall Progress stays at ~95% until M0+ actually ship. |
