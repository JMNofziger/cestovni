...
**Status:** Active — **Stage 5 Step 11 (implementation).** **M0 closed 2026-04-22** — Linear **CES-36** / **CES-37** in **Done**, CTO review passed, `verify-full` green on `main`. **Current execution: M1 / CES-38 in progress** (consumption module Phase 1 landed with fixture harness and `tests/math/` set). **CES-39 is blocked** until [§ M1 prerequisite — UX gap closure](#m1-prerequisite--ux-gap-closure-blocks-ces-39) is satisfied (see [`ux/UX_IMPLEMENTATION_GAPS.md`](ux/UX_IMPLEMENTATION_GAPS.md)). **Repo:** **[CES-53](https://linear.app/personal-interests-llc/issue/CES-53) is Done** (schema v2 + contracts on `main`); **CES-54** is **Done in repo** (2026-04-24, [`DATA_CONTRACTS.md`](ux/DATA_CONTRACTS.md) *Performed time (maintenance)* + `data-model.md` notes); **CES-55** is **Done in repo** (2026-04-25 — `client/lib/app/theme/` with tokens, typography via `google_fonts` deviation, `CestovniTheme.dark()` default, `LedgerCard` / `LedgerTile` / `HairlineDivider`) — **Linear:** mark each **Done** + remove **blocks CES-39** if still attached. Remaining gap-closure blocker for CES-39: **[CES-56](https://linear.app/personal-interests-llc/issue/CES-56)** — confirm **Done** in Linear and that **blocks CES-39** relation matches reality. Parent **CES-35**. M0 follow-ups tracked as **CES-48** (index test tightening), **CES-49** (`settings` DB invariant, blocks CES-42), **CES-50** (remove `_keepMigratorVisible` scaffold, blocks CES-47). M1 follow-ups tracked as **CES-51** (segment cost multi-currency rule) and **CES-52** (adapter dep-direction revisit). Source for Linear `[Delivery v1` epic (CES-35)]([https://linear.app/personal-interests-llc/issue/CES-35/delivery-v1-stage-5-engineering-breakdown](https://linear.app/personal-interests-llc/issue/CES-35/delivery-v1-stage-5-engineering-breakdown)) and per-vertical children. **Granular 🟩/🟨/🟥:** [§ Implementation checklist (RYG)](#implementation-checklist-ryg) + [§ Stage 5 exit criteria (tracking)](#stage-5-exit-criteria-tracking).
...
Linear: each **open** item in `UX_IMPLEMENTATION_GAPS.md` **Critical gaps** is an issue **blocking [CES-39](https://linear.app/personal-interests-llc/issue/CES-39)** (see tracker). The Cestovni team has no dedicated **Blocked** workflow state — **blocking relations** are the source of truth. Do not treat M1 UI rollup as unblocked until **CES-56** is **Done** (CES-53 + CES-54 + CES-55 **Done in repo**; verify Linear + relations).
...
- 🟨 **M1 rollup** — offline logging usable end-to-end without a server.
  ...
  - 🟥 **CES-39 — Fill-up + vehicle UI** — core logging UX; depends on CES-37 + CES-38; **blocked** until **[CES-56](https://linear.app/personal-interests-llc/issue/CES-56)** is **Done** in Linear (CES-53, CES-54, CES-55 **Done in repo** — confirm issue states + relations; tracker: [`ux/UX_IMPLEMENTATION_GAPS.md`](ux/UX_IMPLEMENTATION_GAPS.md)).
...
| 4   | [CES-39](https://linear.app/personal-interests-llc/issue/CES-39) | Fill-up + vehicle UI (core logging)      | M1        | `docs/specs/data-model.md` + `docs/product/PRODUCT_BRIEF.md` + `docs/product/ux/cestovni-views.md` + `docs/product/ux/DATA_CONTRACTS.md` + `docs/product/ux/DELIVERY_ACCEPTANCE.md` + `docs/product/ux/UX_IMPLEMENTATION_GAPS.md` | CES-37, CES-38; **blocked by** [CES-56](https://linear.app/personal-interests-llc/issue/CES-56) (CES-53 + CES-54 + CES-55 **Done** in repo + tracker) | high   | CES-53 + CES-54 + CES-55 landed; awaiting CES-56 + Linear relation cleanup                                                   |
