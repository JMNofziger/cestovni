...
- 🟨 **M1 rollup** — offline logging usable end-to-end without a server. *(Core fill-up path nearly there; Metrics/Maint/photo/settings prefs still open.)*
  ...
  - 🟩 **CES-39 — Fill-up + vehicle UI (core)** — **Done in repo on `main`** (2026-05): repos (phase 1), vehicle CRUD + settings list (phase 2), Log form + draft lifecycle + History list/detail/edit/delete (phase 3); 121+ widget/DB tests green. Unblocks PWA-lite gate ([`pwa-lite-gate.md`](../specs/pwa-lite-gate.md)) M3 client/server slice work. **Still out of CES-39 scope / later:** Metrics + Maint tabs, photo (**CES-40**), History MAINT chip (needs maintenance repo), flip-card History mode (deferred per `DELIVERY_ACCEPTANCE.md`). Settings units/currency/default-vehicle now wired (**CES-57**, done).
...
- 🟨 M0 + M1 land: offline app runs, fill-up works end-to-end, golden math tests green. *(**M0 closed 2026-04-22**. **M1 core done on `main`**: CES-38 + CES-39 (Log/History/repos/vehicle CRUD) Done in repo; Metrics/Maint/photo/settings-prefs still open.)*
