...
- 🟨 **M1 rollup** — offline logging usable end-to-end without a server. *(Core fill-up path done; Metrics/Maint/photo open; Log/History prefs display follow-on from CES-57.)*
  ...
  - 🟩 **CES-39 — Fill-up + vehicle UI (core)** — **Done in repo on `main`** (2026-05): repos (phase 1), vehicle CRUD + settings list (phase 2), Log form + draft lifecycle + History list/detail/edit/delete (phase 3); 121+ widget/DB tests green. Unblocks PWA-lite gate ([`pwa-lite-gate.md`](../specs/pwa-lite-gate.md)) M3 client/server slice work. **Still out of CES-39 scope / later:** Metrics + Maint tabs, photo (**CES-40**), History MAINT chip (needs maintenance repo), flip-card History mode (deferred per `DELIVERY_ACCEPTANCE.md`). **CES-57 follow-on:** wire Log/History to read stored prefs for unit display (not the Settings UI itself).
  - 🟩 **CES-57 — Settings prefs + default vehicle** — **Done in repo on `main`** (2026-06-30, PR #9): schema v3 `default_vehicle_id`, Settings Preferences UI, cold-start seeding in `shell.dart`. Linear: mark **Done** (`scripts/linear-ces57-closeout.mjs`).
